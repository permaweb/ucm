...
test('chg contributor', async () => {
  ...
  const result = await handle(state, {
    ...
    input: { function: 'contributorChg', tier: 'one', target: 'K92n-x2kHiRIBmS0yRGz5ii3OEXFw58__742Qu0DTgA' }
  })
  //console.log(JSON.stringify(result, null, 2))
  assert.equal(result.state.contributors.tiers.one.members['K92n-x2kHiRIBmS0yRGz5ii3OEXFw58__742Qu0DTgA'].amount, 10)
  assert.ok(true)
})
